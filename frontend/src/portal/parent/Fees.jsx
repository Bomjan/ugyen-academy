...
export default function ParentFees() {
  ...
  const totalOwed = fees.filter(f => f.status !== 'paid').reduce((s, f) => s + (f.amount || 0), 0);
  ...
  const byChild = fees.reduce((acc, f) => {
    const name = f.studentName || f.studentId || 'Unknown';
            ...
            {fees.length === 0 ? (
              ...
              Object.entries(byChild).map(([childName, childFees]) => (
                <div key={childName} className="mb-6">
                  <h2 className="text-base font-semibold mb-3">{childName}</h2>
                  <FeeTable fees={childFees} />
                </div>
              ))
            ) : (
              <FeeTable fees={fees} />
