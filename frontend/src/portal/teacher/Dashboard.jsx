  ...
  return (
    <div className="min-h-screen bg-[#0A0A0C] text-white p-6">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-1">Welcome back, {user?.name?.split(' ')[0] || 'Teacher'}</h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-[#141416] border border-white/8 rounded-xl p-4">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-9 h-9 bg-[#0066CC]/15 rounded-xl flex items-center justify-center">
                <Users size={18} className="text-[#0066CC]" />
              </div>
              <span className="text-white/50 text-xs">Total Students</span>
            </div>
            <p className="text-3xl font-bold text-white">{loading ? '—' : students.length}</p>
          </div>
          <div className="bg-[#141416] border border-white/8 rounded-xl p-4">
            <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-lg font-bold text-white">{new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</p>
          </div>
          <div className="bg-[#141416] border border-white/8 rounded-xl p-4">
            <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-lg font-bold text-white">Ugyen Academy</p>
          </div>
        </div>
          ...
          <div className="col-span-2">
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {recent.map((a, i) => (
                  <div key={a._id || i} className="bg-[#141416] border border-white/8 rounded-2xl p-4">
                    <div className="flex items-center gap-2 mb-1">
                      {a.pinned && <Pin size={12} className="text-[#0066CC]" />}
