      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-[#141416] border border-white/8 rounded-2xl p-5 mb-5">
          ...
          {loading ? (
            ...
            <select
              className="w-full bg-[#1C1C1E] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[#0066CC]"
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="bg-[#0066CC] hover:bg-[#0077ED] text-white text-sm px-4 py-2 rounded-xl transition-colors"
              >
            ...
            {showForm && (
              <form onSubmit={handleSave} className="bg-[#141416] border border-white/8 rounded-2xl p-5 mb-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-white/50 mb-1">Subject</label>
                    <select className="w-full bg-[#1C1C1E] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#0066CC]" value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input type="number" min="0" max="100" required className="w-full bg-[#1C1C1E] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#0066CC]" value={form.marks} onChange={e => setForm(f => ({ ...f, marks: e.target.value }))} />
                  ...
                  <div>
                    ...
                    <select className="w-full bg-[#1C1C1E] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#0066CC]" value={form.term} onChange={e => setForm(f => ({ ...f, term: e.target.value }))}>
                      {TERMS.map(t => <option key={t}>{t}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs text-white/50 mb-1">Assessment Type</label>
                    <select className="w-full bg-[#1C1C1E] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#0066CC]" value={form.assessmentType} onChange={e => setForm(f => ({ ...f, assessmentType: e.target.value }))}>
                ...
                <div>
                  ...
                  <textarea className="w-full bg-[#1C1C1E] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-[#0066CC] resize-none" rows={2} value={form.remarks} onChange={e => setForm(f => ({ ...f, remarks: e.target.value }))} />
                </div>
            ...
            {recLoading ? (
              ...
              <div className="bg-[#141416] border border-white/8 rounded-2xl p-8 text-center text-white/30 text-sm">No progress records found for this student.</div>
            ) : (
