...
export default function TeacherAnnouncements() {
  ...
  return (
    <div className="text-white max-w-3xl">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-dark-2 border border-white/[0.05] rounded-2xl p-5 mb-6 space-y-4">
          <div>
            <label className="block text-xs text-white/50 mb-1">Title</label>
            <input required className="w-full bg-dark-3 border border-white/[0.05] rounded-xl px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-accent" placeholder="Announcement title" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} />
          </div>
          <div>
            <label className="block text-xs text-white/50 mb-1">Body</label>
            <textarea required rows={4} className="w-full bg-dark-3 border border-white/[0.05] rounded-xl px-4 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-accent resize-none" placeholder="Write your announcement..." value={form.body} onChange={e => setForm(f => ({ ...f, body: e.target.value }))} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-white/50 mb-1">Target Audience</label>
              <select className="w-full bg-dark-3 border border-white/[0.05] rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-accent capitalize" value={form.targetAudience} onChange={e => setForm(f => ({ ...f, targetAudience: e.target.value }))}>
            ...
            {form.targetAudience === 'class' && (
              <div>
                ...
                <input className="w-full bg-dark-3 border border-white/[0.05] rounded-xl px-3 py-2.5 text-white text-sm placeholder-white/20 focus:outline-none focus:border-accent" placeholder="e.g. Class 8A" value={form.targetClass} onChange={e => setForm(f => ({ ...f, targetClass: e.target.value }))} />
              </div>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-dark-2 border border-white/[0.05] rounded-2xl p-8 text-center text-white/30 text-sm">No announcements yet.</div>
      ) : (
